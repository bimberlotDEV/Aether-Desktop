...
export function SpaceDetailLayout() {
  ...
  return (
    <div className="aether-space-shell">
      {/* Header */}
      <div
        className="aether-space-header"
      ...
      >
        ...
        <div className="flex items-center gap-1">
          ...
          >
            <Star
              size={16}
              fill={space.favourite ? 'var(--color-warning)' : 'none'}
              style={{
                color: space.favourite
                  ? 'var(--color-warning)'
      ...
      {/* Content area */}
      <div className="aether-space-content">
        <Routes>
          <Route
            index
            element={
              <OverviewTab space={space} modules={modules}>
                {children}
              </OverviewTab>
            }
          />
      ...
      {showDelete && (
        <ConfirmDialog
          ...
          confirmLabel={
            children.length > 0
              ? `Delete Space and ${children.length} children`
              : 'Delete'
          }
          danger
          onConfirm={async () => {
      ...
      {children.length > 0 && (
        <section>
          <h2
            ...
            style={{ color: 'var(--color-text-tertiary)' }}
          >
            {space.template_type === 'school' ? 'Subjects' : 'Child Spaces'} (
            {children.length})
          </h2>
        ...
        <div className="space-y-1">
          {modules.map((m) => {
            ...
            return (
              <div
                key={m.id}
                className="aether-surface flex items-center gap-3 px-3 py-3 rounded-xl"
              >
                <span style={{ color: 'var(--color-accent)', display: 'flex' }}>
                  <ModuleIcon size={16} strokeWidth={1.75} />
                </span>
                <span
...
function ModulePlaceholder({ moduleType }: { moduleType: string }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center px-8">
      ...
      >
        <span style={{ color: 'var(--color-accent)', display: 'flex' }}>
          <ModuleIcon2 size={22} strokeWidth={1.75} />
        </span>
      </div>
      <h2
        className="text-lg font-semibold mb-2"
