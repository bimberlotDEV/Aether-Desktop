...
fn input(
    ...
    if e.all_day {
        ...
        return Ok(ExternalEventInput {
            ...
            event_kind: "calendar_event".into(),
            status,
...
pub fn normalize(
    ...
    for e in &events {
        if e.recurrence.is_some() {
            let mut x = input(e, e.recurrence.clone(), &e.start, e.end.as_deref())?;
            x.connection_id = connection_id.into();
            output.push(x);
            continue;
        }
        if e.rrule.is_none() && e.rdates.is_empty() {
            let mut x = input(e, None, &e.start, e.end.as_deref())?;
            x.connection_id = connection_id.into();
            output.push(x);
            continue;
        }
        if e.all_day {
            ...
            for d in result.dates {
                ...
                let mut x = input(e, Some(original.clone()), &original, Some(&end))?;
                x.connection_id = connection_id.into();
                output.push(x);
            }
            continue;
...
pub fn validate(bytes: &[u8]) -> IcsValidation {
    match normalize(bytes, "validation", Utc::now()) {
        Ok(events) => IcsValidation {
            usable: true,
            event_count: events.len(),
            error_code: None,
        },
        Err(code) => IcsValidation {
            usable: false,
            event_count: 0,
...
pub async fn fetch(
    ...
    if let Some(v) = etag {
        request = request.header(reqwest::header::IF_NONE_MATCH, v)
    }
    if let Some(v) = last_modified {
        request = request.header(reqwest::header::IF_MODIFIED_SINCE, v)
    };
