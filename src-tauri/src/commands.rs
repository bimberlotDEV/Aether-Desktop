...
#[tauri::command]
pub fn set_setting(db: State<Database>, key: String, value: String, value_type: Option<String>) -> Result<(), String> {
    with_conn(&db.conn, |conn| repositories::settings::set(conn, &key, &value, &value_type.unwrap_or_else(|| "string".into())))
}
...
pub fn list_settings(db: State<Database>) -> Result<Vec<serde_json::Value>, String> {
    with_conn(&db.conn, |conn| Ok(repositories::settings::list(conn)?.iter().map(json_setting).collect()))
}
...
pub fn get_profile(db: State<Database>) -> Result<Option<serde_json::Value>, String> {
    with_conn(&db.conn, |conn| Ok(repositories::profile::get(conn)?.map(|p| json_profile(&p))))
}

#[tauri::command]
pub fn create_profile(db: State<Database>) -> Result<serde_json::Value, String> {
    let id = Uuid::now_v7().to_string();
    with_conn(&db.conn, |conn| Ok(json_profile(&repositories::profile::create(conn, &id)?)))
}

#[tauri::command]
pub fn update_profile(db: State<Database>, id: String, display_name: Option<String>, onboarding_completed: Option<bool>) -> Result<Option<serde_json::Value>, String> {
    with_conn(&db.conn, |conn| Ok(repositories::profile::update(conn, &id, display_name.as_deref(), onboarding_completed)?.map(|p| json_profile(&p))))
}
...
#[tauri::command]
pub fn create_space(
    db: State<Database>, name: String, description: Option<String>, icon: Option<String>,
    accent: Option<String>, template_type: Option<String>, parent_space_id: Option<String>,
) -> Result<serde_json::Value, String> {
    let space = repositories::spaces::Space {
        id: Uuid::now_v7().to_string(), name, description, icon, accent, template_type,
        favourite: false, archived_at: None, sort_order: 0, settings_json: None,
        parent_space_id, last_opened_at: None,
        created_at: String::new(), updated_at: String::new(),
    };
    with_conn(&db.conn, |conn| Ok(json_space(&repositories::spaces::create(conn, &space)?)))
}
...
pub fn create_space_with_modules(
    ...
    with_conn(&db.conn, |conn| {
        let (space, modules) = repositories::spaces::create_with_modules(conn, &space, &module_refs)?;
        Ok(serde_json::json!({
...
pub fn create_school_space(
    db: State<Database>, name: String, description: Option<String>, icon: Option<String>,
    accent: Option<String>, module_types: Vec<String>, subjects: Vec<serde_json::Value>,
) -> Result<serde_json::Value, String> {
    // subjects is Vec<{ name, icon?, accent? }>
    with_conn(&db.conn, |conn| {
        // Create parent School space
        let school = repositories::spaces::Space {
            id: Uuid::now_v7().to_string(), name, description, icon, accent,
            template_type: Some("school".to_string()),
            favourite: false, archived_at: None, sort_order: 0, settings_json: None,
            parent_space_id: None, last_opened_at: None,
            created_at: String::new(), updated_at: String::new(),
        };
        ...
        for subj in &subjects {
            ...
            let child = repositories::spaces::Space {
                id: Uuid::now_v7().to_string(), name: name.to_string(),
                description: None, icon, accent,
                template_type: Some("subject".to_string()),
                favourite: false, archived_at: None, sort_order: order,
                settings_json: None,
                parent_space_id: Some(school.id.clone()), last_opened_at: None,
                created_at: String::new(), updated_at: String::new(),
            };
            order += 1;
...
#[tauri::command]
pub fn list_spaces(db: State<Database>, include_archived: Option<bool>) -> Result<Vec<serde_json::Value>, String> {
    with_conn(&db.conn, |conn| Ok(repositories::spaces::list(conn, include_archived.unwrap_or(false))?.iter().map(json_space).collect()))
}

#[tauri::command]
pub fn list_top_level_spaces(db: State<Database>) -> Result<Vec<serde_json::Value>, String> {
    with_conn(&db.conn, |conn| Ok(repositories::spaces::list_top_level(conn)?.iter().map(json_space).collect()))
}

#[tauri::command]
pub fn list_child_spaces(db: State<Database>, parent_id: String) -> Result<Vec<serde_json::Value>, String> {
    with_conn(&db.conn, |conn| Ok(repositories::spaces::list_by_parent(conn, &parent_id)?.iter().map(json_space).collect()))
}

#[tauri::command]
pub fn update_space(
    db: State<Database>, id: String, name: Option<String>, description: Option<String>,
    icon: Option<String>, accent: Option<String>, settings_json: Option<String>,
    parent_space_id: Option<Option<String>>,
) -> Result<Option<serde_json::Value>, String> {
    with_conn(&db.conn, |conn| {
        Ok(repositories::spaces::update(conn, &id, name.as_deref(), description.as_deref(), icon.as_deref(), accent.as_deref(), settings_json.as_deref(), parent_space_id)?.map(|s| json_space(&s)))
    })
}

#[tauri::command]
pub fn set_space_modules(db: State<Database>, space_id: String, module_types: Vec<String>) -> Result<Vec<serde_json::Value>, String> {
    let refs: Vec<&str> = module_types.iter().map(|s| s.as_str()).collect();
    with_conn(&db.conn, |conn| Ok(repositories::spaces::set_modules(conn, &space_id, &refs)?.iter().map(json_module).collect()))
}

#[tauri::command]
pub fn get_space_modules(db: State<Database>, space_id: String) -> Result<Vec<serde_json::Value>, String> {
    with_conn(&db.conn, |conn| Ok(repositories::spaces::list_modules(conn, &space_id)?.iter().map(json_module).collect()))
}
...
pub fn delete_space(db: State<Database>, id: String) -> Result<bool, String> {
    with_conn(&db.conn, |conn| repositories::spaces::delete_permanent(conn, &id))
}
...
pub fn duplicate_space(db: State<Database>, id: String) -> Result<serde_json::Value, String> {
    with_conn(&db.conn, |conn| Ok(json_space(&repositories::spaces::duplicate(conn, &id)?)))
}
...
pub fn record_activity(
    db: State<Database>, event_type: String, entity_type: Option<String>,
    entity_id: Option<String>, space_id: Option<String>, metadata_json: Option<String>,
) -> Result<serde_json::Value, String> {
    let event = repositories::activity::ActivityEvent {
        id: Uuid::now_v7().to_string(), event_type, entity_type, entity_id, space_id, metadata_json,
        created_at: String::new(),
    };
    with_conn(&db.conn, |conn| Ok(json_activity(&repositories::activity::record(conn, &event)?)))
}

#[tauri::command]
pub fn list_activity(db: State<Database>, space_id: Option<String>, limit: Option<u32>) -> Result<Vec<serde_json::Value>, String> {
    with_conn(&db.conn, |conn| {
