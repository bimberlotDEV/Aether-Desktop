...
`CAL-SUB-ROTATE-001` is complete. Subscribed-calendar connections now carry a
durable native configuration generation; validated replacement atomically rotates
the encrypted feed credential, resets origin-bound conditional/retry state, and
prevents stale work from reconciling events or updating current lifecycle state.
...
| `cargo test`                                | Pass                 | 2026-09-25    | 186/186 all-feature Rust tests pass with generation-safe replacement coverage. |
